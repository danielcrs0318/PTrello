...
export const TaskModal = ({ open, onClose, task, onTaskUpdate, boardId, isOwner, canEdit = true, initialSubtaskId }) => {
    ...
    const handleCancelEditTitle = () => {
        if (!canEdit) {
            showToast('No tienes permisos para editar tareas en este tablero.', 'warning');
            return;
        }
        setIsEditingTitle(false);
        setEditedTitle(task?.title || '');
    ...
    const handleToggleTaskComplete = async () => {
        if (!task) return;
        if (!canEdit) {
            showToast('No tienes permisos para modificar tareas en este tablero.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (error.response?.status === 403) {
                showToast('No tienes permisos para modificar esta tarea.', 'error');
            }
            setTaskCompleted(!nextCompleted);
        }
    };
    ...
    const handleCreateSubtask = async () => {
        ...
        if (!canEdit) {
            showToast('No tienes permisos para crear subtareas en este tablero.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (error.response?.status === 403) {
                showToast('No tienes permisos para crear subtareas.', 'error');
            }
        } finally {
            setIsCreating(false);
        }
    ...
    const handleToggleSubtask = async (subtask) => {
        if (!canEdit) {
            showToast('No tienes permisos para modificar subtareas en este tablero.', 'warning');
            return;
        }
        try {
            ...
            if (error.response?.status === 403) {
                showToast('No tienes permisos para modificar subtareas.', 'error');
            }
        }
    };
    ...
    const handleDeleteSubtask = async (subtaskId) => {
        if (!canEdit) {
            showToast('No tienes permisos para eliminar subtareas en este tablero.', 'warning');
            return;
        }
        try {
            ...
            if (error.response?.status === 403) {
                showToast('No tienes permisos para eliminar subtareas.', 'error');
            }
        }
    };
    ...
    const handleUploadTaskImages = async (files) => {
        ...
        if (!canEdit) {
            showToast('No tienes permisos para subir imágenes en este tablero.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (error.response?.status === 403) {
                showToast('No tienes permisos para subir imágenes.', 'error');
            }
        } finally {
    ...
    const handleDeleteImage = async (imageId, scope, subtaskId) => {
        if (!imageId) return;
        if (!canEdit) {
            showToast('No tienes permisos para eliminar imágenes en este tablero.', 'warning');
            return;
        }
        try {
            ...
            if (error.response?.status === 403) {
                showToast('No tienes permisos para eliminar imágenes.', 'error');
            }
        }
    };
            ...
            >
                <DialogTitle sx={{
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                        {isEditingTitle ? (
                            <IconButton onClick={handleSaveTitle} sx={{ color: '#4ade80' }} disabled={savingTitle || !canEdit} title="Guardar">
                                <Check />
                            </IconButton>
                        ) : (
                            <IconButton onClick={handleStartEditTitle} sx={{ color: '#9fadbc' }} disabled={!canEdit} title={canEdit ? "Editar tarea" : "Solo lectura"}>
                                <Edit />
                            </IconButton>
                        )}
                        <IconButton onClick={() => setDeleteDialogOpen(true)} sx={{ color: '#ef4444' }} disabled={!canEdit} title={canEdit ? "Eliminar tarea" : "Solo lectura"}>
                            <Delete />
                        </IconButton>
                ...
                <DialogContent sx={{ pt: 2, px: { xs: 2, sm: 3 } }}>
                    <Box sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Checkbox
                            ...
                            disabled={!canEdit || !canToggleTaskComplete()}
                            sx={{
                                color: '#9fadbc',
                    ...
                    <Box sx={{ mb: 3 }}>
                        ...
                        <TextField
                            ...
                            disabled={membersLoading || !isOwner || !canEdit}
                            SelectProps={{
                                multiple: true,
                    ...
                    <Box sx={{ mb: 3 }}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1 }}>
                            <Button
                                ...
                                disabled={savingDescription || (task?.description || '') === description.trim() || !canEdit}
                                variant="contained"
                                sx={{
                                    bgcolor: '#579dff',
                    ...
                    <Box sx={{ mb: 3 }}>
                        ...
                        <Button
                            variant="outlined"
                            disabled={uploadingTaskImages || !canEdit}
                                                        ...
                                                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                                            ...
                                                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                                                                ...
                                                                <Button
                                                                    variant="outlined"
                                                                    component="label"
                                                                    disabled={uploadingSubtaskImages[subtask.id] || !canEdit}
                                                                    sx={{
                                                                        color: '#b6c2cf',
                                                                        borderColor: '#3c434a',
                                                                        textTransform: 'none',
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, mt: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                                <IconButton
                                    ...
                                    disabled={!newSubtaskTitle.trim() || isCreating || !canEdit}
                                    sx={{
                                        bgcolor: '#579dff',
                                        color: 'white',
                                        '&:hover': {
